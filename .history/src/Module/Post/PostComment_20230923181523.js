...
const PostComment = () => {
  ...
  let locationY = null;
  useEffect(() => {
    locationX = refIcon.current.getBoundingClientRect().right;
    locationY = refIcon.current.getBoundingClientRect().bottom;
    console.log(loca);
  }, []);
  return (
    <div className="mb-14">
      ...
      <div className="p-10 w-full relative">
        <div className={`fixed bottom-[${locationY}px] right-[${locationX}px] `}>{show && <Picker onEmojiClick={onEmojiClick} />}</div>
        <textarea
